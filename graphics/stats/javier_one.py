...
def do_plot(post = False) :
    ...
    data_vals = [100, 100]
    bkg_vals = [80, 100]
    bkg_uncerts = [0.1, 0.14]
    if post :
        bkg_vals = [88, 100]
        bkg_uncerts = [0.07, 0.1]
    ...
    ax.text(0.1, 150, "Bkg 1 Pre-fit Uncertainty: 10%", size = 10)

    fig.show()
    outname = "profile_ex_1_NP10_"
    if post :
    	outname += "post"
    else :
    	outname += "pre"
...
def do_pull() :
    ...
    fig.show()
    fig.savefig("profile_ex_1_pulls.pdf", bbox_inches = "tight")
    x = input()
