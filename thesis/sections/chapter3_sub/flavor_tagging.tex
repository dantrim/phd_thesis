...
\begin{itemize}
    ...
    \item{\textbf{Multi-vertex Finding Algorithm (JetFitter, JF)}} The JetFitter algorithm~\cite{JETFITTER} attempts to reconstruct the full
        $b$-hadron decay chain using a modified Kalman filter which assumes a common line on which the primary, $b$-hadron,
...
algorithm was retrained and the training was done using a sample composed of jets both from top-quark
pair production events as well as events from a simulated events of a BSM physics scenario of a heavy $Z^{\prime}$ decaying to $b\bar{b}$.
The latter was included in the re-training so as to allow the algorithm to have in its training sample
...
The set of low-level inputs provided to the MV2 algorithm, per-jet, are listed in Table~\ref{tab:ftag_mv2_inputs}.
Figure~\ref{fig:ftag_mv2c10_disc} shows the MV2 algorithm's output score, which is provided on a per-jet basis.
Working points, defined with different target-efficiencies for accepting $b$-tagged jets, are defined
based on defining thresholds on the MV2 discriminant.
