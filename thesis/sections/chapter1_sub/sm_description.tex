...
\end{equation}
where $\mathbfcal{W}_{\mu}$ is the vector of the three weak gauge fields \fieldWone, \fieldWtwo, and \fieldWthree~and $g_2$ is the weak coupling constant. The non-zero $f^{abc}$ of non-Abelian gauge groups means that the gauge bosons of
...
are typically represented by the Pauli (Gell-Mann) matrices.

The last three terms in Eqn. \ref{eq:sm_lagrangian} are all terms including the Higgs field, $\phi$,
and will be discussed in detail in \ref{sec:higgs_description}.

Inspection of Eqn.~\ref{eq:sm_lagrangian} will reveal two things.
