...
The expected number of events populating each region is then given by the following:
...
\end{align}
where $\bm{\theta}$ are a set of so-called nuisance parameters associated with the systematic
...
since systematic variations can adjust the overall normalisation of a given process (efficiency variations) or adjust
...
The effect of the nuisance parameters associated with each of the analysis' systematic uncertainties
are implemented as Gaussian \textit{constraint terms} in the likelihood defined in Equation~\ref{eq:likelihood_main}:
\begin{align}
...
The test statistic defined for the analyses to be presented are defined based on the following
