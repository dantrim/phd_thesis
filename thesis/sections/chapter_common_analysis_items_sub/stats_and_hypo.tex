...
From this information, a likelihood ratio as in Equation~\ref{eq:full_likelihood} is constructed.
...
By Equation~\ref{eq:theta_gaus}, a pull by 0.5 is precisely 20 events ($\theta^{\text{post-fit}} \times ( \text{pre-fit uncertainty} \times N^{\text{pre-fit}}) \rightarrow 0.5 \times (0.5 \times 80) = 20$).
The post-fit uncertainty has also been reduced to that allowed by the data statistics, which is driven
by the NP on Bkg. 1 being profiled from its initial uncertainty of $\pm 1$ to $\pm 0.25$ ($\Delta \theta^{\text{post-fit}} \times (\text{pre-fit uncertainty} \times N^{\text{pre-fit}}) \rightarrow 0.25 \times (0.5 \times 80) = 10$).
...
are such that they match precision allowed by the data statistics.
...
It is interesting to make note of the fact, exemplified by Example 2 above, that the precision on a given process' $\mu$ term,
