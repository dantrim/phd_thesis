...
Each process' dedicated CR, though, ideally exhibits high purity of the given process and therefore
is the sole CR that has any real constraining power on the process' normalisation correction factor.
If this is true, the expression in Equation~\ref{eq:mu_fac} generally holds true for each process with a
CR, even in the case of multiple
normalisation factors being derived.
%If there is more than one process for which a normalisation correction factor is being derived, and therefore
%more than one CR, the expression given by Equation~\ref{eq:mu_fac} is expanded into a system of
%equations,
